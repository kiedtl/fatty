...
use iced::{
    ...
    advanced::{
        ...
        Shell, Widget, Overlay,
    },
    overlay, touch, window,
    Background, Color, Event, Element, Length, Padding, Pixels, Point, Rectangle,
    Shadow, Size, Vector,
};

use crate::widgets::scrollable::{self, Scrollable};

/// A list of selectable options.
pub struct Menu<'a, 'b, T, Message, Theme = crate::styles::Theme, Renderer = iced::Renderer>
where
    Theme: Catalog,
    ...
    'b: 'a,
{
    ...
    pub fn overlay(
        ...
        overlay::Element::new(Box::new(MyOverlay::new(
            position,
            viewport,
            self,
            target_height,
...
struct MyOverlay<'a, 'b, Message, Theme, Renderer>
where
    Theme: Catalog,
    Renderer: text::Renderer,
{
...
impl<'a, 'b, Message, Theme, Renderer> MyOverlay<'a, 'b, Message, Theme, Renderer>
where
    Message: 'a,
...
impl<Message, Theme, Renderer> Overlay<Message, Theme, Renderer>
    for MyOverlay<'_, '_, Message, Theme, Renderer>
where
    Theme: Catalog,
    Renderer: text::Renderer,
{
...
impl<T, M, Th, Ren> List<'_, '_, T, M, Th, Ren>
where
    T: Clone,
    Th: Catalog,
    Ren: text::Renderer,
{
    pub fn hovered_option(&self) -> Option<usize> {
        (*self.hovered_option)
            .or((self.options.len() > 0).then_some(self.options.len() - 1))
    }

    pub fn maybe_select_hovered(&mut self, shell: &mut Shell<'_, M>) {
    ...
    fn update(
        ...
        match event {
            Event::Keyboard(keyboard::Event::KeyPressed {
                ...
                self.maybe_select_hovered(shell);
            }
