// Copied from iced source.
//
// Copyright iced authors and contributors.
...
use iced::{
    window, border, Background, Border, Color, Element, Length, Padding, Pixels, Point, Rectangle,
    Size, Vector, Theme,
};
    ...
    Renderer: iced::advanced::Renderer,
{
    ...
    pub fn auto_scroll_y(mut self, amount: f32) -> Self {
        self.auto_scroll_y = Some(amount);
        self
    }
    ...
    Renderer: iced::advanced::Renderer,
{
    ...
    fn operate(
        ...
        operation.custom(id, bounds, state);
        operation.scrollable(id, bounds, content_bounds, translation, state);
    ...
    fn update(
        ...
        if let Some(amount) = state.auto_scroll_y
            && let Event::Window(window::Event::RedrawRequested(_)) = event {
                if let Some(i) = state.last_auto_scrolled_y {
                    println!("Last scrolled {}ms ago", i.elapsed().as_millis());
                }
                state.last_auto_scrolled_y = Some(Instant::now());

                state.scroll(
                    self.direction.align(Vector::new(0.0, amount)),
                    bounds,
                    content_bounds,
                );

                shell.request_redraw_at(Instant::now() + Duration::from_millis(1000 / 40));
        ...
        let mut update = || {
                ...
                || !matches!(event, Event::Mouse(mouse::Event::WheelScrolled { .. }))
            {
                ...
                let cursor = match cursor_over_scrollable {
                    ...
                    // _ => cursor.levitate() + translation, //mouse::Cursor::Unavailable,
...
pub struct State {
    ...
    offset_y: Offset,
    auto_scroll_y: Option<f32>,
    last_auto_scrolled_y: Option<Instant>,
...
impl State {
    ...
    pub fn new(auto_scroll_y: Option<f32>) -> Self {
        Self {
            scroll_area_touched_at: None,
            offset_y: Offset::Absolute(0.0),
            auto_scroll_y,
